...
import { getOrders, getStatusLabel, publishListing, deleteOrders, setDealerWebsiteStatus, reseedDemoData } from '@/lib/orderApi'
...
export function OrdersPage() {
  ...
  const buildLocalFallback = () => {
    ...
    for (let i = 0; i < 48; i++) {
      ...
      const buyerName = inventoryStatus === 'SOLD' ? `Buyer ${i + 1}` : ''
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      ...
      {/* Global search + Bulk actions */}
        ...
        <input
          className="flex-1 min-w-[280px] max-w-md border border-gray-300 rounded px-3 py-2 bg-white outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
        ...
        <div className="flex items-center gap-2">
          <button
            className="px-2 py-1 border rounded bg-white inline-flex items-center gap-2 text-sm whitespace-nowrap w-44 justify-center"
          ...
          <button ref={customizeBtnRef} className="px-2 py-1 border rounded bg-white inline-flex items-center gap-2 text-sm whitespace-nowrap w-44 justify-center" onClick={() => setCustomizeOpen(true)} aria-expanded={customizeOpen} title="Manage Columns">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
          ...
          {selectedIds.size > 0 && (
            <button
              className="px-2 py-1 border rounded bg-white inline-flex items-center gap-2 disabled:opacity-50 text-sm whitespace-nowrap w-44 justify-center"
              disabled={deleting}
      ...
      <div className="overflow-x-auto border rounded" style={{ maxHeight: `${tableMaxHeight}px`, overflowY: pageSize > 10 ? 'auto' : 'hidden' }}>
      ...
      <div className="flex items-center justify-between mt-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="px-3 py-1 border rounded bg-white disabled:opacity-50" disabled={page <= 1} onClick={() => setPage(p => Math.max(1, p - 1))}>Previous</button>
          <button className="px-3 py-1 border rounded bg-white disabled:opacity-50" disabled={page >= totalPages} onClick={() => setPage(p => Math.min(totalPages, p + 1))}>Next</button>
        </div>
      </div>
    </div>
  )
